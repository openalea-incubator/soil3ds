...
def test_soil_coupling1():
    foldin =  'inputs' #os.path.join(path_, 'inputs')
    fxls_sol = 'Parametres_sol_exemple.xls'
    ongletS = 'lusignan99'

    path_sol = os.path.join(foldin,fxls_sol)
    ...
    path_plante = os.path.join(foldin,'Parametres_plante_exemple.xls')#'Parametres_plante_v5cLucas.xls')#'Parametres_plante_v18.xls')#'Parametres_plante_v9Lucas_debugL.xls')#r'H:\devel\grassland\grassland\L-gume\Parametres_plante_v5cLucas.xls'
    ongletP = 'Fix2'
    ...
    for j in range(n_jour):
        ...
        if j > 0:
            assert uptNO3Pltmole < uptNO3Pltmole_old
            uptNO3Pltmole_old = uptNO3Pltmole
        else:
            uptNO3Pltmole_old = uptNO3Pltmole
        # print de sorties W
